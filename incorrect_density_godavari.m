...
clear all
close all
%location of true depth for Godavari basin
    ...
    density1=@(z) (-0.4692*exp(-4.078*10^-4*z)).*1000+((-0.4692*exp(-4.078*10^-4*z)).*1000)*0.1;
    %function for correct density contrast 
    density2=@(z) (-0.4692*exp(-4.078*10^-4*z)).*1000;
    %maximum depth bound
    max_depth=6000;
    ...
    %patch plot for true model 
     ...
     title('Depth profile of Godavari basin with incorrect density')
     ...
     title('Gravity anomaly of Godavari Basin with incorrect density')
