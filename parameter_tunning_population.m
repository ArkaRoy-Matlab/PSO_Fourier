% ***************************************************************
% *** Matlab function for parameter tuning for particle population of the swarm using PSO
...
%% Problem Definition
c1=1.4; c2=1.7;  %c1 and c2 of PSO 
%loop for Particle population tunning 
nPoPP=10:10:100;
    for ip=1:length(nPoPP)
            ...
            nPoP =  nPoPP(ip);     %Population size or swarm size
            ...
            costt(ip)=mean_beast(1);
            timee(ip)=mean_beast(2);
            iterr(ip)=floor(mean_beast(3));
            %printing results for all population variation
            fprintf('For npop= %d, best cost =%f, best time==%2.2f and number of iteration=%d\n',nPoP,mean_beast(1),mean_beast(2),mean_beast(3))
    end
